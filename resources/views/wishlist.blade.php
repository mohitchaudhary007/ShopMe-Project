                        ...
                        <tr>
                            <td>
                                <div class="shopping-cart__product-item">
                                    <img loading="lazy" 
                                         src="{{ $item->model->image ? asset('uploads/products/thumbnails/' . $item->model->image) : asset('images/placeholder.png') }}" 
                              ...
                              <div class="row">
                                ...
                                <div class="col-6">
                                  <form action="{{ route('wishlist.item.remove', ['rowId' => $item->rowId]) }}" method="POST" id="remove-item-{{ $item->id }}">
                                      @csrf
                                      @method("DELETE")
                                      <a href="javascript:void(0)" class="remove-cart" title="Remove item from wishlist" onclick="document.getElementById('remove-item-{{ $item->id }}').submit();">
